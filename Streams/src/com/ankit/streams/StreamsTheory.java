...
// In case of filter data may not be reduce.


// we have few numbers of objects in collections, we don't process the data directly  
// in the collection,it is a source,we will add this data
// objects into the stream and we will apply all the mechanism on the streaming element
// once you added/ join the element in the stream then we can apply filter, map and some 
// other methods on this particular data, but the source will remains same , we were not modifying the source
